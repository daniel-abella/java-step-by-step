...
//		String nomeDriver = "sun.jdbc.odbc.JdbcOdbcDriver";
//		String url = "jdbc:odbc:sisBanco";
//		String user = "admin";
//		String pwd = "";
		RepositorioClientes repClientes = new RepositorioClientesArray();
		Cliente c = new Cliente("1","Cliente Teste");
		try {
			repClientes.inserir(c);
		} catch (ErroAcessoRepositorioException e) {
		}
//		RepositorioClientes repClientes =
//			new RepositorioClientesBDR(nomeDriver, url, user, pwd);
		clientes = new CadastroClientes(repClientes);
		
		RepositorioContas rep = new RepositorioContasArray();
		try {
			rep.inserir(new Conta("1", 1000,c));
		} catch (ErroAcessoRepositorioException e) {
		}
//		RepositorioContas rep =
//			new RepositorioContasBDR(nomeDriver, url, user, pwd);
	 ...
	 *            seu método chamador.
	 * @exception ErroAcessoRepositorioException lançada quando ocorre erro de acesso ao
	 *            mecanismo de armazenamento de dados implementado no repositório.Esta exceção 
	 ...
	 *            seu método chamador.
	 * @exception ErroAcessoRepositorioException lançada quando ocorre erro de acesso ao 
	 *            banco de dados de contas. Esta exceção vem da chamada ao cadastro de 
	 *            contas e é repassada diretamente por este método ao seu método chamador.  
	 */
	...
	public Collection pesquisarTodasContas() throws ContaInexistenteException,
												  ErroAcessoRepositorioException {
		return this.contas.pesquisarTodasContas();
	}
